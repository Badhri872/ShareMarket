...
namespace Services.WebSocket
{
    public class AliceBlue
    {
        ...
        readonly Dictionary<string, string> _urls = new Dictionary<string, string>
        {
            ...
            ["contract.master.json"] = "https://v2api.aliceblueonline.com/restpy/contract_master?exch={EXCHANGE}",
        ...
        private void CreateWebsocketSession()
        {
            ...
            {
                if (_enableLogging)
                    Utilities.LogMessage($"Error creating websocket session.{ex.Message}");
            }
        }

        private string GetWebsocketAccessToken()
        {
            return Utilities.GetSHA256(Utilities.GetSHA256(_accessToken));
        }
        ...
        public async Task<T> GetMasterContracts<T>(string exchange) where T : class
        {
            T content = null;
            using (HttpClient httpClient = new HttpClient())
            {
                ...
                var url = _urls["contract.master.json"].Replace("{EXCHANGE}", exchange.ToUpper());
        ...
        public async Task<List<T>> GetMasterContractDetails<T>(string url) where T : class
        {
            ...
            using (HttpClient httpClient = new HttpClient())
            {
                ...
                using (var response = await httpClient.GetAsync(url))
                {
                    ...
                    var jsonString = await response.Content.ReadAsStringAsync();
                    content = JsonConvert.DeserializeObject<List<T>>(jsonString);
                }
            }
            return content;
        }
                    ...
                    while (csv.ReadNextRecord())
                    {
                        try
                        {
                            contracts.Add(new Contract
                            {
                                ...
                                LotSize = csv.HasHeader("Lot Size") && !string.IsNullOrEmpty(csv["Lot Size"]) ? csv["Lot Size"] : " ",
                                TickSize = csv.HasHeader("Tick Size") && !string.IsNullOrEmpty(csv["Tick Size"]) ? csv["Tick Size"] : " ",
                            });
                        }
                        catch (Exception ex)
                        {
                            if (_enableLogging) Utilities.LogMessage(ex.ToString());
                        }
                    }
                }
            });
        ...
        {
            return Execute<T>(endpoint, inputParams, Method.Post).Result;
        }
        public T ExecuteGet<T>(string endpoint, object inputParams = null) where T : class
        {
            return Execute<T>(endpoint, inputParams, Method.Get).Result;
        }

        protected async Task<T> Execute<T>(string endpoint, object inputParams = null, Method method = Method.Get) where T : class
        {
            var request = new RestRequest(endpoint);

            if (inputParams != null)
                request.AddStringBody(Utilities.Serialize(inputParams), ContentType.Json);
            ...
            if (response != null && !string.IsNullOrEmpty(response.ErrorMessage) && _enableLogging)
                Utilities.LogMessage($"Error executing api request. Status: {response.StatusCode}-{response.ErrorMessage}");
