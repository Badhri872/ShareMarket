...
namespace Services.WebSocket
{
    internal class AliceBlueAuthenticator : AuthenticatorBase
    {
        ...
        {
            if (_enableLogging)
                Utilities.LogMessage("Getting the Access Token (Session Id).");
            ...
            if (_enableLogging)
                Utilities.LogMessage($"Calling encryption key endpoint: {_encryptionKeyEndpoint}");

            var encryptionKeyResponse = await restClient.PostAsync<EncryptionKeyResult>(request);

            if (_enableLogging)
                Utilities.LogMessage($"Encryption Key Result. Status: {encryptionKeyResponse.Status}-{encryptionKeyResponse.ErrorMessage}");
            ...
            var userData = Utilities.GetSHA256($"{_userId}{_apiKey}{encryptionKeyResponse.EncryptionKey}");
            ...
            if (_enableLogging)
                Utilities.LogMessage($"Calling create session endpoint: {_sessionIdEndpoint}");
            ...
            if (createSessonResponse.Status == Constants.API_RESPONSE_STATUS_Not_OK)
                if (_enableLogging)
                    Utilities.LogMessage($"Error creating sesson. Status: {createSessonResponse.Status}, Error Message: {createSessonResponse.ErrorMessage}");
                else if (_enableLogging)
                    Utilities.LogMessage($"Create Session Result. Status: {createSessonResponse.Status}-{createSessonResponse.ErrorMessage}");
